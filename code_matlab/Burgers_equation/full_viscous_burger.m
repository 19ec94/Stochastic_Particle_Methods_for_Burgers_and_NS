...
type_burger = menu('Choose the equation:','Inviscid Burgers equation','Viscid Burgers equation');
if (type_burger == 1)
...
ictype = menu('Choose the initial condition type:', ...
    'Piecewise constant (shock)','Piecewise constant (expansion)','Gaussian','sinus','Piecewise continuous', ...
    'direc delta');
%Selection of numerical parameters (time step, grid spacing, etc...).
xend = 1; % x-axis size.
tend = 0.2; % t-axis size.
